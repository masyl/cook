...
function rawTag(context, frame) {
	var str = "",
		args = this.args(context);
	if (args) {
		str = args[0];
		if (frame.$removeWhitespaces) str = str.trim();
		return str;
	}
	return "";
}
...
function bindTag(context, frame) {
	...
	if (args && args.length > 0) {
		...
		if (!frame.$bindings) frame.$bindings = [];
		frame.$bindings.push(key);
...
function elemTag(context, frame) {
	...
	if (frame.$bind) {
		...
		binding = " data-binding='" + frame.$bindings.join("::") + "'";
		frame.$bind = false;
	}
...
function elseTag(context, frame) {
	var args,
		isUndefinedOrTrue, // Will be true if there are no arguments or if the first one is "not false" 
		str = "";
	frame.$ifIsTrue = false;

	//TODO : THIS WONT WORK!!! $ifContinued should be on the parent frame

	if (this.tags && frame.$ifContinued) {
		args = this.args(context);
		isUndefinedOrTrue = (args === void 0 || args && args[0]);
		if (isUndefinedOrTrue) {
			str = this.tags.render(context, frame);
...
function Partial(name, tags, argNames, context, frame) {
	...
	this.render = function () {
		var output;
		// Transfer the arguments passed to the partial into named arguments into the frame
		for (var i = 0; i < argNames.length; i++) {
			if (i < arguments.length) frame[argNames[i]] = arguments[i];
		}
		output = this.tags.render(context, frame);
		return output;
	};
}
