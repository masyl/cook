...
function autoTag(context, frame) {
	...
	if (this.autoIsTag) {
		if (context[0].$templates[this.name]) {
...
function setTag(context, frame) {
	...
	if (args.length > 0) {
		if (this.tags) {
			value = frame.$body = this.tags.render(context, frame);
		}
...
function globalTag(context, frame) {
	var value,
		args = this.args(context);
	if (args.length > 0) {
		if (this.tags) {
		...
		frame.$root.$globals[args[0]] = value;
	}
	return "";
}
 ...
 * @return {String}
 */
function templateTag(context, frame) {
	var args, name, templateHandler, template;
	if (this.tags) {
		args = this.args(context);
		if (args && args.length) {
			name = args.shift();
			template = new Template(name, this.tags, args, context, frame);
			...
			context[0].$templates[name] = templateHandler;
		}
	}
	return "";
}

function importTag(context, frame) {
	renderTag.call(this, context, frame);
	return "";
}
...
function renderTag(context, frame) {
	...
	if (args && args.length) {
		name = args.shift();
		template = context[0].$templates[name];
		...
		if (typeof template === "function" && template.$isTemplate) {
			...
			// Render the template tag with the new frame
 ...
 * @constructor
 */
function Template(name, tags, argNames, context, frame) {
	var template = this;
	this.name = name;
	this.tags = tags;
	this.render = function () {
		var newFrame = this;
		// Transfer the arguments passed to the template into named arguments into the new frame
		...
		return template.tags.render(_context, newFrame);
	};
}
