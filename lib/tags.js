...
function defaultTag(data) {
	...
	if (scopeMember) {
		...
		return "<" + this.name.substring(1) + binding + ">" + str + "</" + this.name.substring(1) + ">";
	}
}
...
function autoTag(data) {
	...
	if (type == "object") {
		...
		return defaultTag.call(this, data);
	}
}
...
function eachTag(data) {
	...
	if (this.tags) {
		...
		if (args && args.length) {
			source = args[0];
			valueName = (args.length > 1) && args[1];
			...
			for (i = 0; i < source.length; i++) {
				...
				if (valueName) data[valueName] = data.$value;
