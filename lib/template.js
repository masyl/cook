module.exports = function (cook) {
	...
	return function Template(cook) {
		...
		this.renderDOM = function renderDOM(model) {
			if (cook.options.HTMLDocument) {
				var document = new cook.options.HTMLDocument();
				return this.render(model, "dom", document);
			}
		};

		this.renderObject = function renderObject(model) {
			return this.render(model, "object");
		};

		// Return a function that that will be used to render the template
		this.render = function render(model, _mode, document) {
			var mode = _mode || "text";
			var output;
			// Create a new evaluation context instance with the supplied model
			var context = new Context(model, mode, document);
			...
			output = this.handler(context);
			if (mode === "dom") {
				template.output = document.createElement("document");
				if (output) {
					for (var i = 0; i < output.length; i++) {
						template.output.appendChild(output[i]);
					}
				}
			} else {
