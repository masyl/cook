...
(function () {
	...
	function main(template) {
		...
		tokens = lexer(template, options.start, options.end);
		root = builder(tokens);
		return function render(data) {
			data.__removeWhitespaces = false;
	...
	function builder(tokens) {
		...
		while (tokens.length > 0) {
			...
			if (isInsideTag) {
				...
				split = tagContentString.substring(name.length).split(options.filter);
				args = split[0];
				filter = split[1];
	...
	function Tag(name, argString, filterString) {
		...
		this.render = function(data) {
			...
			if (this.toggleWhitespace) data.__removeWhitespaces = !data.__removeWhitespaces;
	...
	function rawTag(data) {
		...
		if (args) {
			str = args[0];
			if (data.__removeWhitespaces) str = str.trim();
			return str;
		}
		return "";
	}
		...
		if (template) {
			...
			for (i = 0; i < splits.length; i++) {
				...
				if (closing >= 0) {
					...
					token = template.substring(closing + options.end.length, splits[i + 1]); // todo: refactor: Is this line the same as above ???
					tokens.push([
						false,
						false,
						token
