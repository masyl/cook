(function () {
		...
		while (tokens.length > 0) {
			...
			if (isInsideTag) {
				...
				if (isSelfClosing) {
					...
					tag = new Tag(name, args, filter, true);
				...
				tag = new Tag("raw", tagContentString, filter, false, true);
				stack[0].tags.unshift(tag);
			}
		}
		return root;
	 ...
	 * @constructor
	 */
	function Tag(name, argString, filterString, isAltTag, isRaw) {
		this.name = name;
		this.tags = [];
		...
		if (this.filterString && this.filterString.trim() !== "") {
			this.filter = funex("__Array(" + this.filterString + ")");
		} else {
			//todo: optimize, when no filter simply do nothing.
			this.filter = function (data) { return data.body };
		}

		this.render = function(data) {
			//todo: raise syntax error for invalid/unknown tags
			var output = this.handler.call(this, data);
			data.body = output;
