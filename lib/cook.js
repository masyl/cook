(function () {
	...
	function dali2(options) {
		...
		/**
		 * Render a remplate into an executable function
		 * @param template
		 * @return {*}
		 */
		function main(template) {
			// todo: no support yet for delimiters of variable length
			var tokens = lexer(template, startDelimiter, endDelimiter);
	...
	//			exitList = result[2];
				if (tag) {
					...
					if (tag[0][0] === endTagDelimiter) {
						return [tagSet, tag]; // todo: refactor:  Or use exitList boolean
					}
					if (firstTag) {
						if (tag[0].substring(0, firstTag[0].length + 1) === firstTag[0] + alternateTagDelimiter) { // todo: refactor: use indexOf ?
		...
		function parseTag(tokens, index) {
			...
			// If this token opens a tag
				if (token[0]) {
					...
					tagExpressions = content.substring(name.length).split(filtersDelimiter);
					...
					if (!token[1] && name[0] !== endTagDelimiter) {
						childs = parseTagList(tokens, index);
						return [[name, args, filters, childs], false, (name[0] !== endTagDelimiter)];
					} else {
		...
		function lexer(template, start, end) {
			...
			for (i = 0; i < splits.length; i++) {
				...
				if (closing >= 0) {
					token = template.substring(splits[i]+2, closing).trim();
					isSelfClosing = (token[token.length-1] === endTagDelimiter);
					...
					token = template.substring(closing+2, splits[i+1]).trim(); // todo: refactor: Is this line the same as above ???
					tokens.push([
						false,
						false,
						token
