	...
	function lexer(template, start, end) {
		...
		if (template) {
			...
			for (i = 0; i < splits.length; i++) {
				...
				if (closing >= 0) {
					token = template.substring(splits[i] + options.startDelimiter.length, closing);
					...
					token = template.substring(closing + options.endDelimiter.length, splits[i + 1]); // todo: refactor: Is this line the same as above ???
					tokens.push([
						false,
						false,
						token
